...
| PR-009 | Immutable production consumption (GHCR digest) | **BLOCKED** | `docker build -t ghcr.io/seanchatmangpt/ggen-ecosystem:v26.8.28 .` succeeded (same cached layers as PR-004's image, pack-tree/binary identity reverified — see `receipts/local-qualification-20260828.json`). Real `docker push` refused: `error from registry: permission_denied: The token provided does not match expected scopes` — the `gh` OAuth token has `repo,workflow,read:org,gist,project` but not `write:packages`. Requires the account holder to run `gh auth refresh -h github.com -s write:packages,read:packages` (interactive device-flow, cannot be completed non-interactively) then re-authenticate `docker login ghcr.io`. |
...
| PR-016 | Local GitHub Actions replay (`act`) | **PARTIAL_ALIVE** | `.actrc` present (maps `ubuntu-24.04`, forces `linux/amd64`). `act --list` real output confirms both `construct` and `build-and-push` jobs are discoverable. Real execution attempted 2026-08-28 against the locally-built `ghcr.io/seanchatmangpt/ggen-ecosystem:v26.8.28` image (`act workflow_dispatch -j construct ... --container-architecture linux/arm64`, required since the local build is arm64 and act's default amd64 platform has no local image to run): job setup reached `docker create`/`docker run` for the job container, then failed with `error while creating mount source path '/Users/sac/.colima/default/docker.sock': mkdir ...: operation not supported` — act's job-container docker-socket bind-mount is incompatible with Colima's non-default socket path on this host. This is a real, host-specific environment defect, not a repo defect; a real GitHub-hosted `ubuntu-24.04` runner (standard `/var/run/docker.sock`) is not expected to hit it. Unresolved this session — would need either a Docker-Desktop-style default socket path or an act/Colima socket-symlink workaround, neither attempted yet. |
...
14 + 5 + 1 + 1 = 21, matching the full `PR-001`–`PR-021` set exactly. Updated 2026-08-28: real
`docker push` of `ghcr.io/seanchatmangpt/ggen-ecosystem:v26.8.28` attempted with explicit user
confirmation and refused by the registry (`permission_denied`, token missing `write:packages`) —
PR-009 downgraded from `UNKNOWN` to the more precise `BLOCKED` (the exact fix is now named, not
merely "not yet attempted"). A real `act workflow_dispatch -j construct` execution against that
locally-built image was also attempted; it reached the job container's `docker run` step and hit
a Colima-specific docker-socket bind-mount defect (`mkdir .../docker.sock: operation not
supported`) — a host environment limitation, not a repo defect, and PR-016 stays
`PARTIAL_ALIVE` with the root cause now exact instead of "neither job executed yet". (Prior
update, retained below: after build6
